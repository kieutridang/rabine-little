...
const LandscapeRepairInfo = ({
  ...
  const zoneLayer = find(layers, (l) => l._id === zoneId);
  ...
  return (
    <StyledPopup info position={position} onClose={onClose}>
      <StyledForm onSubmit={() => false}>
        ...
        <FormRow>
          <FormColumn>
            <FormLabel info labelColumn fontSize="0.75rem">TITLE (REPAIR TYPE)</FormLabel>
            <FormRowContent info noMargin>{repairType}</FormRowContent>
          </FormColumn>
        </FormRow>
...
LandscapeRepairInfo.defaultProps = {
  readOnly: false,
};


LandscapeRepairInfo.propTypes = {
...
export default LandscapeRepairInfo;
