...
import { API_DRONEPARTNER_PATH } from '../../constants';
...
export function* fetchDronePartner() {
  try {
    const response = yield call(rabineFetcher.get, API_DRONEPARTNER_PATH);
...
export function* addDronePartner(action) {
  try {
    const response = yield call(
      rabineFetcher.post,
      API_DRONEPARTNER_PATH,
      action.payload.dronePartner
    );
