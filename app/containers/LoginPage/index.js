...
import { Form, Input, InputWrap, Logo, Article, HighlightLink, Button, Container, ErrorMessage } from './StyledComponents';
      ...
      <Article>
        ...
        <Container>
          <Form onSubmit={handleSubmit}>
            ...
            <HighlightLink><FormattedMessage {...messages.dontHaveAccount} /> <Link to="/signup"><FormattedMessage {...messages.createAccountLink} /></Link></HighlightLink>
          </Form>
        </Container>
      </Article>
    );
...
export function mapDispatchToProps(dispatch) {
  return {
    onSubmitForm: (username, password) => {
      dispatch(loginToAuthenticate(username, password));
    },
  };
}
...
const loginFormik = withFormik({
  ...
  handleSubmit: (values) => {
    values.submitForm(values.email, values.password);
  },
});
